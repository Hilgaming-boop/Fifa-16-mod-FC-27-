...
function events:publishHomeEventsList()
  ...
  local homeLineup = self.homeTeamlineupData or {}
  local awayLineup = self.awayTeamlineupData or {}

  -- 🔹 Gabungkan kedua tim, tapi beri tanda asal tim
  local allPlayers = {}
  ...
  -- 🔹 Ambil maksimal 14 pemain dari masing-masing tim
  local selectedPlayers = {}
  local homeCount, awayCount = 0, 0
  for _, player in ipairs(allPlayers) do
    if player.isHome and homeCount < 14 then
      table.insert(selectedPlayers, player)
      homeCount = homeCount + 1
    elseif not player.isHome and awayCount < 14 then
      table.insert(selectedPlayers, player)
  ...
  for _, player in ipairs(selectedPlayers) do
    ...
    -- 🔹 Tentukan offset crest & icon sesuai tim (biar posisi beda)
    local iconOffset = player.isHome
        and (DEFAULT_ICON_HOME_OFFSET + self.mAdditionalIconOffset)
        or  (DEFAULT_ICON_AWAY_OFFSET or -DEFAULT_ICON_HOME_OFFSET)
    ...
    table.insert(eventList, {
      data = {
        ...
        isHome = player.isHome and 1 or 0
      }
    })
  end

  -- Jika ingin urut berdasarkan rating (opsional)
  -- table.sort(eventList, function(a, b)
  --   return (tonumber(a.data.Rating) or 0) > (tonumber(b.data.Rating) or 0)
  -- end)
