...
local TableUtil = (...)
local BND_FORMATION_INDEX = "bnd_formation_index"
local BND_FORMATION_LIST = "bnd_formation_list"
local BND_FORMATION = "bnd_formation"
local ACT_FORMATION_CHANGE = "act_change"
local ACT_FORMATION_POPUP = "act_formation_popup"
...
function FormationModel:new(init)
    ...
    -- Verify this is definitely the away team
    o.isAwayTeam = o.services.GameSetupService.GetTeamId(false) == o.teamID
    
    -- Initialize formation data
    o.formationNamesList = {}
    o.formationList = o:getFormationList()
    o.matchFormationList = o:getMatchFormationList()
    o.currentFormationIndex = 1
    
    -- Away team specific formation setup
    if o.teamID ~= nil then
        local formationID = 0
        if o.gamemode == "fut" then
            -- FUT mode handling (same for both teams)
            ...
            -- Standard mode - exclusively for away team (teamSide = 1)
            if o.isAwayTeam then
                o.currentFormationID = o.services.TacticsService.GetFormation(1, o.teamID)
                formationID = o.currentFormationID
            else
                error("This FormationModel is for away team only")
            end
        end
    ...
    o.im.RegisterDataAction(BND_FORMATION_INDEX, ACT_FORMATION_CHANGE, function(bindingName, actionName, formationIndex)
        if o.currentFormationIndex ~= formationIndex + 1 then
            ...
            if o.onFormationChangeCallback then
    ...
    o.im.Subscribe(BND_FORMATION_INDEX, function() o:_publishFormationIndex() end)
    o.im.Subscribe(BND_FORMATION_LIST, function() o:_publishFormationList() end)
    o.im.Subscribe(BND_FORMATION, function() o:_publishFormation() end)
    o.im.RegisterAction(ACT_FORMATION_POPUP, function() o:formationPopup() end)
    
    return o
end

-- The following functions remain exactly the same as home version 
-- since they're team-agnostic
function FormationModel:getFormationList()
    ...
    for i = 1, #formationIDs do
        formationList[i] = {
            id = formationIDs[i],
            name = formationNames[i].name,
            coords = formationCoords[i],
            positions = formationPositions[i]
        }
        self.formationNamesList[i] = { text = formationList[i].name }
    end
    
    return formationList
end

function FormationModel:getMatchFormationList()
    local formationList = {}
    local formationIDs = self.services.SquadManagementService.GetAvailableFormationIDs(self.gamemode == "fut")
    local formationNames = self.services.TacticsService.GetAllFormationNames(self.gamemode == "fut")
    local formationCoords = self.services.TacticsService.GetAllFormationCoords()
    local formationPositions = self:getMatchFormationPositions()
...
function FormationModel:_publishFormationIndex()
  self.im.Publish(BND_FORMATION_INDEX, self.currentFormationIndex - 1)
end
...
function FormationModel:_publishFormation()
  self.im.Publish(BND_FORMATION, self.formationList[self.currentFormationIndex])
end
...
function FormationModel:finalize()
  ...
  self.im.UnregisterDataAction(BND_FORMATION_INDEX, ACT_FORMATION_CHANGE)
  self.im.Unsubscribe(BND_FORMATION_INDEX)
  self.im.Unsubscribe(BND_FORMATION_LIST)
  self.im.Unsubscribe(BND_FORMATION)
  self.im.UnregisterAction(ACT_FORMATION_POPUP)
end
