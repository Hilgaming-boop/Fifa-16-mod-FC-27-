...
function TeamManagementModel:new(init)
  ...
  local currentLeague = o:getCurrentLeague()
    o.im.Publish("bnd_bg_pitch", {name = "$Pitch_", id = currentLeague})
  end)
  ...
  o.im.Subscribe("bnd_table_career", function() end)
...
local function getSharpnessIcons(sharpnessName)
    if not sharpnessName then return "0xFFFFFF" end
local icons = {
    ["99"]  = "$Sharpness_4",   ["83"]  = "$Sharpness_4",   ["65"]  = "$Sharpness_3",   
    ["98"]  = "$Sharpness_4",   ["80"]  = "$Sharpness_4",   ["60"]  = "$Sharpness_3",   
    ["95"]  = "$Sharpness_4",   ["79"]  = "$Sharpness_4",   ["58"]  = "$Sharpness_2",   
    ["94"]  = "$Sharpness_4",   ["77"]  = "$Sharpness_4",   ["55"]  = "$Sharpness_2",   
    ["93"]  = "$Sharpness_4",   ["75"]  = "$Sharpness_3",   ["54"]  = "$Sharpness_2",   
    ["92"]  = "$Sharpness_4",   ["74"]  = "$Sharpness_3",   ["52"]  = "$Sharpness_2",   
    ["90"]  = "$Sharpness_4",   ["73"]  = "$Sharpness_3",   ["49"]  = "$Sharpness_1",   
    ["88"]  = "$Sharpness_4",   ["70"]  = "$Sharpness_3",   ["43"]  = "$Sharpness_1",   
    ["86"]  = "$Sharpness_4",   ["68"]  = "$Sharpness_3",   ["35"]  = "$Sharpness_1", 
    ["85"]  = "$Sharpness_4",    ["67"]  = "$Sharpness_3", 
    ["84"]  = "$Sharpness_4",    ["66"]  = "$Sharpness_3",   
}
...
local function getFitnessIcons(fitnessName)
    if not fitnessName then return "0xFFFFFF" end
local icons = {
    ["99"]  = "$FitnessGreen",   ["84"]  = "$FitnessGreen",   ["73"]  = "$FitnessYellow",   ["58"]  = "$FitnessOrange",   ["45"]  = "$FitnessRed",
    ["98"]  = "$FitnessGreen",   ["83"]  = "$FitnessGreen",   ["72"]  = "$FitnessYellow",   ["57"]  = "$FitnessOrange",   ["43"]  = "$FitnessRed",
    ["96"]  = "$FitnessGreen",   ["82"]  = "$FitnessGreen",   ["70"]  = "$FitnessYellow",   ["56"]  = "$FitnessOrange",   ["40"]  = "$FitnessRed",
    ["95"]  = "$FitnessGreen",   ["81"]  = "$FitnessGreen",   ["69"]  = "$FitnessYellow",   ["55"]  = "$FitnessOrange",   ["37"]  = "$FitnessRed",
    ["94"]  = "$FitnessGreen",   ["80"]  = "$FitnessGreen",   ["68"]  = "$FitnessYellow",   ["54"]  = "$FitnessOrange",   ["35"]  = "$FitnessRed",
    ["91"]  = "$FitnessGreen",   ["79"]  = "$FitnessGreen",   ["66"]  = "$FitnessYellow",   ["53"]  = "$FitnessOrange",   ["33"]  = "$FitnessRed",
    ["90"]  = "$FitnessGreen",   ["78"]  = "$FitnessGreen",   ["65"]  = "$FitnessYellow",   ["52"]  = "$FitnessOrange",   ["32"]  = "$FitnessRed",
    ["89"]  = "$FitnessGreen",   ["77"]  = "$FitnessGreen",   ["64"]  = "$FitnessYellow",   ["51"]  = "$FitnessOrange",   ["30"]  = "$FitnessRed",
    ["88"]  = "$FitnessGreen",   ["76"]  = "$FitnessGreen",   ["62"]  = "$FitnessYellow",   ["49"]  = "$FitnessOrange",   ["27"]  = "$FitnessRed",
    ["86"]  = "$FitnessGreen",    ["75"]  = "$FitnessYellow",   ["60"]  = "$FitnessYellow",   ["48"]  = "$FitnessOrange",   ["25"]  = "$FitnessRed",
    ["85"]  = "$FitnessGreen",    ["74"]  = "$FitnessYellow",   ["59"]  = "$FitnessOrange",   ["46"]  = "$FitnessOrange",   ["20"]  = "$FitnessRed",
}
    ...
    LW =  "RW/ST"   
        ...
        local sharpnessIcons = getSharpnessIcons(sharpnessName)
            ...
            self.im.Publish("bnd_active_player_flags", {
                name = "$Flag64x64",
                id = activePlayer.countryID or 0
            })           
...
function TeamManagementModel:publishSwapAssets()
  ...
  self.im.Publish("bnd_swap3_active", {
    ...
    self.im.Publish("bnd_divider_swap", "$Divider")
    self.im.Publish("bnd_table_career", {name = "$Table_", id = self:getCurrentLeague()})
    self.im.Publish("bnd_vertical_swap", "$UserPlateDividerLine")
    self.im.Publish("bnd_rectangle_swap", "$RectangleSwap")
    self.im.Publish("bnd_active_player_playstyles", "$PlayStyle") 
...
function TeamManagementModel:_publishStarting11()
  ...
  for i = 1, MAX_STARTING do
    ...
    if p then
      local copy = TableUtil.remove_flag_and_crest(p)      
      starting11[i] = copy
    end
  end
...
function TeamManagementModel:_publishSubsAndRes()
    ...
    local subsAndRes = {}
    for i = 1, nSubs do
        local p = self.players[i + MAX_STARTING]
        if p then
            local copy = TableUtil.remove_flag_and_crest(p)
            subs[i] = copy
        end
    ...
    if nRes > 0 then
     for i = 1, nRes do
        local p = self.players[i + MAX_STARTING + nSubs]
        if p then
            local copy = TableUtil.remove_flag_and_crest(p)
            res[i] = copy
        end
...
function TeamManagementModel:_publishTeamRating()
    local team = self.teamID
    if not team then 
    ...
    local teamRating = {
        attackValue = self.teamInfo.offense,
        middleValue = self.teamInfo.midfield,
        defenseValue = self.teamInfo.defense,
        attackLabel = self.loc.LocalizeString("LTXT_CMN_ATT"),
        middleLabel = self.loc.LocalizeString("LTXT_CMN_MID"),
        defenseLabel = self.loc.LocalizeString("LTXT_CMN_DEF")
    }
...
function TeamManagementModel:clearPlayerVisibility()
    ...
    self.im.Publish("bnd_active_player_rating", 0)
    self.im.Publish("bnd_active_player_avatar", {
        name = "$Head",
        id = 0
    })
    ...
    self.im.Publish("bnd_passive_player_rating", 0)
    self.im.Publish("bnd_passive_player_avatar", {
        name = "$Head",
        id = 0
    })
