-- Career Mode Davun --
-- Calendar Remode By Sabil - Septiawan --
...
function Calendar:publishPlayerInfo()
    ...
    if not selectedItem.image then
        ...
        self.im.Publish("bnd_teamcrest", teamcrest)
        self.im.Publish("bnd_date", selectedItem.month and (selectedItem.month .. " " .. selectedItem.day) or date)
        self.im.Publish("bnd_teamname", teamname)
        self.im.Publish("bnd_result", result)
        self.im.Publish("bnd_simtext", simtext)
        self.im.Publish("bnd_simicon", simicon)
    ...
    local function getDayName(y, m, d)
        if m < 3 then
            m = m + 12
            y = y - 1
        end
    ...
    local mNum = monthNumMap[selectedItem.month] or 1
    local shortMonth = monthShortMap[selectedItem.month] or selectedItem.month
    local dayName = getDayName(tonumber(selectedItem.year), mNum, tonumber(selectedItem.day))
    date = string.format("%s, %s %d", dayName, shortMonth, tonumber(selectedItem.day))
    ...
    local teamName = self.loc and self.loc.LocalizeString("TeamName_Abbr15_" .. currentSelectedTeamID) or "Team " .. currentSelectedTeamID
    teamcrest = {name = "$Crest", id = currentSelectedTeamID}
    teamname = teamName  -- ubah dari "No match scheduled" ke nama tim yang dipilih
    simtext = isPastOrCurrentDate and "" or "Sim to Date"
    simicon = isPastOrCurrentDate and {} or "$SimMatchIcon2"
    ground = "N/A"
...
function Calendar:getPlayers()
  ...
  local GRIDS_PER_SET = 35 
  local dateStr = GLOBAL_DATE_PLACEHOLDER or "01/08/24" 
  ...
  local monthDays = {}
  for i = 0, 11 do
    local monthIndex = (month - 1 + i) % 12 + 1
    local baseMonth = baseMonths[monthIndex]
    local monthYear
    if monthIndex >= 8 then 
      monthYear = oldYear
    else 
      monthYear = newYear
    end
...
for _, monthInfo in ipairs(monthDays) do
  ...
  self.playersDataToPublish = { index = 3, data = gridData }
