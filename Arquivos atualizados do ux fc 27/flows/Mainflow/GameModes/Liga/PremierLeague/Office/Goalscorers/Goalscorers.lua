...
local bndgoalScoreCareer = "bnd_goalscore_career"
...
function Liga:new(init)
  ...
  o.im.Subscribe(bndgoalScoreCareer, function()  
    o.im.Publish(bndgoalScoreCareer, {name = "$BackgroundOffice", id = IDLeague})   
  end)
  
  o.im.Subscribe(bndLogo, function()  
    o.im.Publish(bndLogo, {name = "$LeagueLogo2", id = IDLeague})   
  end)
...
function Liga:publishMatchRows()
    ...
    for rank, player in ipairs(topPlayers) do
        ...
        local teamName = self.loc.LocalizeString("TeamName_Abbr15_" .. player.teamID)
