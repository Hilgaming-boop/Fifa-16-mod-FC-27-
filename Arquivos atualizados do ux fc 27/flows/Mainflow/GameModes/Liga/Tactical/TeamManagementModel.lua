...
function TeamManagementModel:new(init)
    ...
    o.im.Publish(bndBackgroundCareer, {name = "$BgsLl", id = currentSelectedTeamID})   
  end)
...
function TeamManagementModel:getPlayers(teamID)
    ...
    if not players or #players == 0 then
        error("[TeamManagementModel]: No players found for teamID " .. teamID)
    end
    ...
    function swapPlayersInTeam(teamPlayers, cardIDToSwap)
        ...
        -- If player is already outside starting 11, no swap needed
        if playerToSwapIndex > 11 then
            print("[TeamManagementModel]: Player " .. cardIDToSwap .. " is already a reserve, no swap needed")
            return true
        end

        -- Look for a player outside indices 1-11 with the same position
    ...
    function transferPlayer(playerID, sourceTeamID, targetTeamID)
        ...
        -- Swap player to reserves if in starting 11 (for currentSelectedTeamID)
        if sourceTeamID == currentSelectedTeamID then
            ...
            if not swapSuccess then
                -- If no swap is possible, proceed with removal but log it
        ...
        -- Remove the player (now guaranteed to be a reserve if swapped)
        ...
        -- Get target team lineup independently
        ...
        -- Update players if teamID matches source or target
    ...
    -- Handle incoming transfer (player bought by currentSelectedTeamID)
    if teamID == currentSelectedTeamID and GLOBAL_PLAYERIN and GLOBAL_TEAMBUY then
        transferPlayer(GLOBAL_PLAYERIN, GLOBAL_TEAMBUY, currentSelectedTeamID)
    elseif teamID == currentSelectedTeamID then
        print("[TeamManagementModel]: GLOBAL_PLAYERIN or GLOBAL_TEAMBUY is nil, skipping incoming transfer.")
    end

    -- Handle outgoing transfer (player sold from currentSelectedTeamID)
    if GLOBAL_REPLAYER and teamID == currentSelectedTeamID then
        transferPlayer(GLOBAL_REPLAYER, currentSelectedTeamID, GLOBAL_TEAMSELL)
    end

    local benchPlayers = {}
    for index = 12, #players do
        table.insert(benchPlayers, players[index])
    end
