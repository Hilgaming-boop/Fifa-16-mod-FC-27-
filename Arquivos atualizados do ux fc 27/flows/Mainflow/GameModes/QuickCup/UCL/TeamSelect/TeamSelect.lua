...
local teamToCountryMap = {
    ...
    [155611] = 160, [155612] = 160, [155614] = 160
}
...
local TeamDatabase = {
    [1]  = { 18,13,5,219,483,448,325,2014,819,113888,48,111282,280,1824,918,245,110919,266,73,243,10,21,9,44,22,241,1,32,240,234,39,45,231,247,237,69,126987 },
    [3]  = { 2,14,25,36,52,65,66,72,78,86,189,191,209,211,236,246,320,326,393,449,450,673,896,900,919,1516,1874,1884,1896,1903,100545,100632,100761,101078,110468,112372 },
...
function TeamSelect:new(init)
    ...
    for i = 1, 32 do
        o.im.RegisterAction("act_tab_team" .. i, function() 
            ...
            updateTeamVisualsByIndex(i) -- Update info tim (rating/grup/dll)
            updateTabVisibility(i)      -- Update visibilitas tab (highlighter)
        end)
    end
    ...
    if currentTourInfo[tourId] and currentTourInfo[tourId].homeID ~= 0 then
        ...
        if TeamListData[1] then
            ...
            updateTabVisibility(1) -- Munculkan tab 1 sebagai default
        end
        ...
        o.im.RegisterAction(ACT_RANDOM_TEAMS, function()
             o:RandomizeTeams()
             updateTeamVisualsByIndex(1)
             updateTabVisibility(1) -- Reset ke tab 1 setelah acak
        end)
    end
    
    print("✅ TeamSelect initialized: Tab Visibility 1-32 Integrated")
    return o
end
