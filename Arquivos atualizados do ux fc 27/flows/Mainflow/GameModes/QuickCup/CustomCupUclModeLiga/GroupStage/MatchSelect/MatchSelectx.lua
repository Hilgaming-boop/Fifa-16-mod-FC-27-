...
function QuickTour:GenerateNextKnockoutRound(teams)
    ...
    for i = 1, #teams, 2 do
        ...
        print("⚽ "..nextRound..": "..teams[i].." vs "..teams[i+1])
    end
...
function QuickTour:GenerateKnockoutFromGroupStandings()
    ...
    if not GroupStandings or not GroupStandings[tourId] then
        print("❌ Group standings not found")
        return
    end

    QuickTourGrouping[tourId] = QuickTourGrouping[tourId] or {}

    print("🔀 Generating Round of 16 (FIXED BRACKET)")

    -- 🔹 Ambil semua tim
    local allTeams = {}
    for _, groupData in pairs(GroupStandings[tourId]) do
        for _, t in pairs(groupData) do
    ...
    local P = {}
    for i = 1, 16 do
        P[i] = allTeams[i].teamId
        print("Qualified P"..i.." = "..P[i])
    end
