...
-- KONFIGURASI MATCHDAY BARU --
local currentMatchday = 1
local totalMatchdays = 8 -- Biasanya 8 matchday untuk format baru, sesuaikan jika perlu
local matchesPerMatchday = 18 -- Diubah dari 16 menjadi 18
...
function Standings:setupSubscriptions()
    self.im.Subscribe(BND_COLOR_CUPID, function()
        ...
        local color1 = tourIdToColorMap[currentTourId] or tourIdToColorMap.default
        self.im.Publish(BND_COLOR_CUPID, color1)
    end)

    self.im.Subscribe(BND_COLOR_CUPID2, function()
        local currentTourId = GlobalTournamentSettings.tourId or 1
        local color2 = (tourIdToColorMap2 and tourIdToColorMap2[currentTourId])
    ...
    {"bnd_matchday_label",
     ...
     "bnd_realtime",
     "bnd_tour_label",
     "bnd_champions_team_name",
     "bnd_bg_tour",
     "bnd_tour_logo",
     "bnd_group_label_current",
     "bnd_group_label_next",
     "bnd_group_label_prev",
     "bnd_tour_trophy",
     "bnd_champions_crest"
    }

    for _, label in ipairs(infoLabels) do
        self.im.Subscribe(label, function()
...
-- LOGIKA MATCHDAY FIXTURES --
-- LOGIKA MATCHDAY FIXTURES (DISATUKAN) --
function Standings:publishCurrentGroupFixtures()
    ...
    local lastDetectedMatchday = 0

    -- Iterasi seluruh data pertandingan di QuickTourGrouping
    for i, match in ipairs(QuickTourGrouping[tourId]) do
        -- Pastikan ini adalah fase grup (index 8 true)
        if match and match[8] == true then 
            
            -- Hitung matchday berdasarkan indeks (asumsi 16 match per matchday)
            local matchdayOfThisMatch = math.floor((i - 1) / matchesPerMatchday) + 1
            
            -- Jika matchday berubah, masukkan item Header/Keterangan
            if matchdayOfThisMatch ~= lastDetectedMatchday then
                table.insert(fixturesData, {
                    data = {
                        MatchDay = "MATCHDAY " .. matchdayOfThisMatch,
                        IsHeader = true, -- Gunakan flag ini untuk membedakan UI di template
                        FontColor = "0xFFFF00" -- Warna khusus header
                    }
                })
                lastDetectedMatchday = matchdayOfThisMatch
            end
...
function Standings:initializeTournament()
    ...
    -- Inisialisasi Grup (Hanya untuk perhitungan Klasemen)
    for g = 1, 8 do
...
function Standings:publishCurrentGroupStandings()
    -- Klasemen tetap berdasarkan grup yang berisi player (League Stage / Group Stage)
    local playerGroup = 1
    local playerTeamID = currentTourInfo[tourId] and currentTourInfo[tourId].homeID or 0
    
    for gIndex, group in ipairs(self.groups) do
        for _, teamID in ipairs(group.teams) do
            if teamID == playerTeamID then playerGroup = gIndex; break end
        end
    end

    local group = self.groups[playerGroup]
    ...
    table.insert(uiData, {
        data = {
            Teampos = " POS", TeamPoint = "PTS", Teammp = "P", TeamWin = "W", TeamDraw = "D",
            TeamLoss = "L", TeamGA = "GF", TeamGC = "GA", TeamGD = "GD", FontColor = "0x0C0C0C", IsHeader = true
        }
    })

    for pos, team in ipairs(group.standings) do
        local isPlayerTeam = (team.teamID == playerTeamID)
        local fontColor = isPlayerTeam and "0xFFD700" or "0x333333"
        ...
        table.insert(uiData, {
            data = {
                ...
                TeamPoint = team.points, Teammp = team.played, TeamWin = team.wins,
                TeamDraw = team.draws, TeamLoss = team.losses, TeamGA = team.goalsFor,
                TeamGC = team.goalsAgainst, TeamGD = goalDiff, Teampos = pos .. ".",
                FontColor = fontColor, Icon = isPlayerTeam and { name = "$my_team" } or nil
            }
        })
    end
...
function Standings:publishAllStandings()
    -- Logika klasemen seluruh tim (misal untuk League Format)
    local allTeams = {}
    for groupIndex = 1, #self.groups do
        local group = self.groups[groupIndex]
        if group then
            local stats = self:calculateGroupStandings(group)
            for _, team in ipairs(stats) do table.insert(allTeams, team) end
        end
    end
    ...
    for pos, team in ipairs(allTeams) do
        local isPlayer = (team.teamID == playerTeamID)
        table.insert(standingsData, { data = {TeamCrest = { name = "$Crest32x32", id = team.teamID },TeamName = self.loc.LocalizeString("TeamName_Abbr15_" .. (team.teamID or 0)),TeamPoint = team.points, Teammp = team.played, Teampos = pos .. ".",FontColor = isPlayer and "0xFFD700" or "0x333333"}})
    end
...
-- UPDATE LABEL UNTUK MATCHDAY --
function Standings:publishGroupLabel()
    local prevMD = currentMatchday - 1
    if prevMD < 1 then prevMD = totalMatchdays end
    local nextMD = currentMatchday + 1
    if nextMD > totalMatchdays then nextMD = 1 end

    self.im.Publish("bnd_group_label_current", "MATCHDAY " .. currentMatchday)
    self.im.Publish("bnd_group_label_prev", "MATCHDAY " .. prevMD)
    self.im.Publish("bnd_group_label_next", "MATCHDAY " .. nextMD)
end

function Standings:publishTournamentProgress()
    local progressText = "Group Stage"
    -- Logika progress stage (Knockout) tetap sama
...
function Standings:NextGroup()
    ...
    if currentMatchday > totalMatchdays then currentMatchday = 1 end
    self:publishCurrentGroupFixtures()
    self:publishGroupLabel()
    self.im.Publish("bnd_matchday_label", "MATCHDAY " .. currentMatchday)
end

function Standings:PrevGroup()
    currentMatchday = currentMatchday - 1
    if currentMatchday < 1 then currentMatchday = totalMatchdays end
    self:publishCurrentGroupFixtures()
    self:publishGroupLabel()
    self.im.Publish("bnd_matchday_label", "MATCHDAY " .. currentMatchday)
end
