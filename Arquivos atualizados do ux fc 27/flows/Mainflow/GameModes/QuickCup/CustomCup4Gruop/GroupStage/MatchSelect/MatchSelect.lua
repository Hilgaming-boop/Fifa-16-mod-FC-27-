...
function QuickTour:GenerateKnockoutFromGroupStandings()
    ...
    local QF_MATCHDAY = lastGroupMatchday + 1
    ...
    -- Mengambil data grup (Hanya A, B, C, D untuk 16 tim)
    local groupsData = {}
    ...
    for _, letter in ipairs(letters) do
        ...
        -- Pastikan grup memiliki data sebelum dimasukkan
        if #t >= 2 then
            groupsData[letter] = t -- t[1]=Juara, t[2]=Runner-Up
        else
            print("⚠️ Warning: Data Group " .. letter .. " tidak lengkap.")
            return
        end
    end

    -- Struktur Bagan Quarter Final (8 Tim):
    -- QF1: Juara A vs Runner-up B
    -- QF2: Juara B vs Runner-up A
    -- QF3: Juara C vs Runner-up D
    -- QF4: Juara D vs Runner-up C
    local pairings = {
        {groupsData["A"][1], groupsData["B"][2]}, -- QF1
        {groupsData["B"][1], groupsData["A"][2]}, -- QF2
        {groupsData["C"][1], groupsData["D"][2]}, -- QF3
        {groupsData["D"][1], groupsData["C"][2]}  -- QF4
    }
    ...
    for _, pair in ipairs(pairings) do
        table.insert(QuickTourGrouping[tourId], {  
            ...
            0, 0, false, 0,  
            true, false, "Quarter Final", QF_MATCHDAY  
        })
    end
    print("✅ Quarter Final Generated: 16-team format (Top 2 from Groups A-D).")
end
