...
local matchlabel = "bnd_match_label"

-- ★ NEW: dua binder warna
...
function Tour:setupSubscriptions()
    ...
    self.im.Subscribe(matchlabel, function()
        local tourName = tourIdToNameMap[self.tourId] or tourIdToNameMap.default
        self.im.Publish(matchlabel, tourName)
    end)
        ...
        if designatedScorer then
            for _, p in ipairs(players) do
                ...
                if goals > 0 and p.position == "GK" then 
                    print("⚠️ GK Mencetak gol: " .. tostring(pid) .. " → Dialihkan ke penyerang/pemain terbaik: " .. tostring(designatedScorer.CARD_ID))
...
function Tour:finalize()
    ...
    self.im.Unsubscribe(matchlabel)

    -- ★ NEW: unsubscribe dua warna
