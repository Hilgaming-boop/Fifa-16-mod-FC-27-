...
function Standings:publishCurrentGroupFixtures()
    ...
    -- ===============================
    -- TAMPILKAN SAMPAI MATCHDAY 6
    -- 1 Matchday = 2 Match
    -- 6 Matchday = 12 Match
    -- ===============================
    local maxMatchesToShow = 12 
    
    -- LOOP PER MATCHDAY (loncat 2 karena 1 matchday = 2 match)
    for mIndex = 1, maxMatchesToShow, 2 do
        ...
        -- LOOP SEMUA GRUP (A-H / sesuai totalGroups)
        for gIndex = 1, totalGroups do
            ...
            for i = 0, 1 do
                ...
                if match then
                    local isPlayerMatch =
                        (match.homeID == playerTeamID or match.awayID == playerTeamID)

                    local scoreText =
                        match.isPlayed and
                        (match.homeScore .. " - " .. match.awayScore) or "VS"

                    local homeNameRaw =
                        self.loc.LocalizeString("TeamName_Abbr15_" .. (match.homeID or 0))
                    local awayNameRaw =
                        self.loc.LocalizeString("TeamName_Abbr15_" .. (match.awayID or 0))
