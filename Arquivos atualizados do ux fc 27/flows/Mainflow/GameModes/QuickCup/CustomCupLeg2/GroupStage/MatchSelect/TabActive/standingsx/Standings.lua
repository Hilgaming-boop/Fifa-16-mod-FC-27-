...
local BND_CUP_MAP_TOURID = "bnd_cup_map_tourid"
...
local bndChampionsTeamCrest = "bnd_champions_team_crest"
local bndChampionsLabel = "bnd_champions_label"
local bndR16TeamNames = {}
for i = 1, 16 do
    bndR16TeamNames[i] = "bnd_r16_team_name" .. i
end
local bndQFTeamNames = {}
for i = 1, 8 do
    bndQFTeamNames[i] = "bnd_qf_team_name" .. i
end
local bndSFTeamNames = {}
for i = 1, 4 do
    bndSFTeamNames[i] = "bnd_semifinal_team_name" .. i
end
local bndFinalTeamNames = {}
for i = 1, 2 do
    bndFinalTeamNames[i] = "bnd_final_team_name" .. i
end
-- =========================
-- KNOCKOUT TEAM CRESTS
-- =========================
local bndR16TeamCrests = {}
for i = 1, 16 do
    bndR16TeamCrests[i] = "bnd_r16_team_crest" .. i
end

local bndQFTeamCrests = {}
for i = 1, 8 do
    bndQFTeamCrests[i] = "bnd_qf_team_crest" .. i
end

local bndSFTeamCrests = {}
for i = 1, 4 do
    bndSFTeamCrests[i] = "bnd_semifinal_team_crest" .. i
end

local bndFinalTeamCrests = {}
for i = 1, 2 do
    bndFinalTeamCrests[i] = "bnd_final_team_crest" .. i
end
-- =========================
-- KNOCKOUT TEAM SCORES
-- =========================
local bndR16TeamScores = {}
for i = 1, 16 do
    bndR16TeamScores[i] = "bnd_r16_team_score" .. i
end

local bndQFTeamScores = {}
for i = 1, 8 do
    bndQFTeamScores[i] = "bnd_qf_team_score" .. i
end

local bndSFTeamScores = {}
for i = 1, 4 do
    bndSFTeamScores[i] = "bnd_semifinal_team_score" .. i
end

local bndFinalTeamScores = {}
for i = 1, 2 do
    bndFinalTeamScores[i] = "bnd_final_team_score" .. i
end
...
function Standings:setupSubscriptions()
    ...
    self.im.Subscribe(bndChampionsTeamName, function()self:publishChampions()end)
    self.im.Subscribe(bndChampionsTeamCrest, function()self:publishChampions()end)
    self.im.Subscribe(bndChampionsLabel, function()self:publishChampions()end)
    for i = 1, 16 do
    local idx = i
    self.im.Subscribe(bndR16TeamNames[i], function()
        self:publishR16TeamNames()
    end)
end
    for i = 1, 8 do
    local idx = i
    self.im.Subscribe(bndQFTeamNames[i], function()
        self:publishQFTeamNames()
    end)
end
    for i = 1, 4 do
    local idx = i
    self.im.Subscribe(bndSFTeamNames[i], function()
        self:publishSFTeamNames()
    end)
end
for i = 1, 2 do
    local idx = i
    self.im.Subscribe(bndFinalTeamNames[i], function()
        self:publishFinalTeamNames()
    end)
end
    -- =========================
-- CREST SUBSCRIPTIONS
-- =========================
for i = 1, 16 do
    self.im.Subscribe(bndR16TeamCrests[i], function()
        self:publishR16TeamCrests()
    end)
end

for i = 1, 8 do
    self.im.Subscribe(bndQFTeamCrests[i], function()
        self:publishQFTeamCrests()
    end)
end

for i = 1, 4 do
    self.im.Subscribe(bndSFTeamCrests[i], function()
        self:publishSFTeamCrests()
    end)
end

for i = 1, 2 do
    self.im.Subscribe(bndFinalTeamCrests[i], function()
        self:publishFinalTeamCrests()
    end)
end
-- =========================
-- SCORE SUBSCRIPTIONS
-- =========================
for i = 1, 16 do
    self.im.Subscribe(bndR16TeamScores[i], function()
        self:publishR16TeamScores()
    end)
end

for i = 1, 8 do
    self.im.Subscribe(bndQFTeamScores[i], function()
        self:publishQFTeamScores()
    end)
end

for i = 1, 4 do
    self.im.Subscribe(bndSFTeamScores[i], function()
        self:publishSFTeamScores()
    end)
end

for i = 1, 2 do
    self.im.Subscribe(bndFinalTeamScores[i], function()
        self:publishFinalTeamScores()
    end)
end
    local infoLabels = {
 "bnd_matchday_label", "bnd_team_label", "bnd_month_label",
 "bnd_realtime", "bnd_tour_label", "bnd_bg_tour",
 "bnd_tour_logo", "bnd_cup_map_tourid",  -- ✅ NEW
 "bnd_group_label_current", "bnd_group_label_next",
 "bnd_group_label_prev", "bnd_tour_trophy"
}
...
function Standings:publishHomeInfo()
    ...
    self.im.Publish("bnd_cup_map_tourid", { name = "$CupMap", id = tourId }) -- ✅ NEW
...
function Standings:publishR16TeamNames()
    local r16Teams = {}
        ...
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.R16 then
                ...
                if match[1] and match[1] ~= 0 then
                    table.insert(r16Teams, match[1])
                end
                -- Ambil away
                if match[2] and match[2] ~= 0 then
                    table.insert(r16Teams, match[2])
                end
            end
        end
    end

    -- Kalau R16 belum ada → kosongkan semua
    if #r16Teams == 0 then
        for i = 1, 16 do
            self.im.Publish(bndR16TeamNames[i], "")
        end
        return
    end

    -- Publish sampai 16 tim
    for i = 1, 16 do
        local teamID = r16Teams[i]
        if teamID then
            local name = self.loc.LocalizeString("TeamName_Abbr3_" .. teamID)
            self.im.Publish(bndR16TeamNames[i], name)
        else
            self.im.Publish(bndR16TeamNames[i], "")
        end
    end
...
function Standings:publishQFTeamNames()
    local qfTeams = {}
    ...
    if QuickTourGrouping and QuickTourGrouping[tourId] then
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.QF then
                if match[1] and match[1] ~= 0 then
                    table.insert(qfTeams, match[1])
                end
                if match[2] and match[2] ~= 0 then
                    table.insert(qfTeams, match[2])
                end
            end
        end
    end

    -- Kalau QF belum ada → kosongkan semua
    if #qfTeams == 0 then
        for i = 1, 8 do
            self.im.Publish(bndQFTeamNames[i], "")
        end
        return
    end
    ...
    -- Publish 8 tim
    for i = 1, 8 do
        local teamID = qfTeams[i]
        if teamID then
            local name = self.loc.LocalizeString("TeamName_Abbr3_" .. teamID)
            self.im.Publish(bndQFTeamNames[i], name)
        else
            self.im.Publish(bndQFTeamNames[i], "")
        end
    end
...
function Standings:publishSFTeamNames()
    local sfTeams = {}
    ...
    if QuickTourGrouping and QuickTourGrouping[tourId] then
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.SF then
                if match[1] and match[1] ~= 0 then
                    table.insert(sfTeams, match[1])
                end
                if match[2] and match[2] ~= 0 then
                    table.insert(sfTeams, match[2])
                end
            end
        end
    end

    -- Kalau semifinal belum ada → kosongkan semua
    if #sfTeams == 0 then
        for i = 1, 4 do
            self.im.Publish(bndSFTeamNames[i], "")
        end
        return
    end
    ...
    -- Publish 4 tim
    for i = 1, 4 do
        local teamID = sfTeams[i]
        if teamID then
            local name = self.loc.LocalizeString("TeamName_Abbr3_" .. teamID)
            self.im.Publish(bndSFTeamNames[i], name)
        else
            self.im.Publish(bndSFTeamNames[i], "")
        end
    end
...
function Standings:publishFinalTeamNames()
    local finalTeams = {}
    ...
    if QuickTourGrouping and QuickTourGrouping[tourId] then
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.F then
                if match[1] and match[1] ~= 0 then
                    table.insert(finalTeams, match[1])
                end
                if match[2] and match[2] ~= 0 then
                    table.insert(finalTeams, match[2])
                end
            end
        end
    end

    -- Kalau Final belum ada → kosongkan
    if #finalTeams == 0 then
        for i = 1, 2 do
            self.im.Publish(bndFinalTeamNames[i], "")
        end
        return
    end
    ...
    -- Publish 2 tim final
    for i = 1, 2 do
        local teamID = finalTeams[i]
        if teamID then
            local name = self.loc.LocalizeString("TeamName_Abbr3_" .. teamID)
            self.im.Publish(bndFinalTeamNames[i], name)
        else
            self.im.Publish(bndFinalTeamNames[i], "")
        end
    end
...
-- R16 CRESTS
-- =========================
function Standings:publishR16TeamCrests()
    local teams = {}

    if QuickTourGrouping and QuickTourGrouping[tourId] then
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.R16 then
                if match[1] and match[1] ~= 0 then table.insert(teams, match[1]) end
                if match[2] and match[2] ~= 0 then table.insert(teams, match[2]) end
            end
        end
    end
    ...
    for i = 1, 16 do
        ...
        if teamID then
            self.im.Publish(bndR16TeamCrests[i], { name = "$Crest", id = teamID })
        else
            self.im.Publish(bndR16TeamCrests[i], nil)
        end
    end
end
...
-- QF CRESTS
-- =========================
function Standings:publishQFTeamCrests()
    local teams = {}
    ...
    for i = 1, 8 do
        local teamID = teams[i]
        if teamID then
            self.im.Publish(bndQFTeamCrests[i], { name = "$Crest", id = teamID })
        else
            self.im.Publish(bndQFTeamCrests[i], nil)
        end
    end
end
...
-- SEMIFINAL CRESTS
-- =========================
function Standings:publishSFTeamCrests()
    local teams = {}
    ...
    for i = 1, 4 do
        local teamID = teams[i]
        if teamID then
            self.im.Publish(bndSFTeamCrests[i], { name = "$Crest", id = teamID })
        else
            self.im.Publish(bndSFTeamCrests[i], nil)
        end
    end
end
...
-- FINAL CRESTS
-- =========================
function Standings:publishFinalTeamCrests()
    local teams = {}
    ...
    for i = 1, 2 do
        local teamID = teams[i]
        if teamID then
            self.im.Publish(bndFinalTeamCrests[i], { name = "$Crest", id = teamID })
        else
            self.im.Publish(bndFinalTeamCrests[i], nil)
        end
    end
end

function Standings:publishR16TeamScores()
    local scores = {}

    if QuickTourGrouping and QuickTourGrouping[tourId] then
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.R16 then
                table.insert(scores, match[3] or 0) -- home
                table.insert(scores, match[4] or 0) -- away
            end
        end
    end

    for i = 1, 16 do
        self.im.Publish(bndR16TeamScores[i], scores[i] or "")
    end
end

function Standings:publishQFTeamScores()
    local scores = {}

    if QuickTourGrouping and QuickTourGrouping[tourId] then
        for _, match in ipairs(QuickTourGrouping[tourId]) do
            if match[9] == STAGE_NAMES.QF then
                table.insert(scores, match[3] or 0)
                table.insert(scores, match[4] or 0)
            end
        end
    end

    for i = 1, 8 do
        self.im.Publish(bndQFTeamScores[i], scores[i] or "")
    end
end

function Standings:publishSFTeamScores()
    local scores = {}
    ...
    for i = 1, 4 do
        self.im.Publish(bndSFTeamScores[i], scores[i] or "")
    end
end

function Standings:publishFinalTeamScores()
    local scores = {}
    ...
    for i = 1, 2 do
        self.im.Publish(bndFinalTeamScores[i], scores[i] or "")
    end
end

function Standings:publishChampions()
    local championTeamID = nil
    ...
    -- Kalau belum ada juara
    if not championTeamID then
        self.im.Publish(bndChampionsLabel, "")
        self.im.Publish(bndChampionsTeamName, "")
        self.im.Publish(bndChampionsTeamCrest, nil)
        return
    end

    -- Publish LABEL + JUARA
    self.im.Publish(bndChampionsLabel, "THE CHAMPION IS")

    self.im.Publish(
        bndChampionsTeamName,
        self.loc.LocalizeString("TeamName_Abbr15_" .. championTeamID)
    )

    self.im.Publish(
        bndChampionsTeamCrest,
        { name = "$Crest", id = championTeamID }
    )
end
