...
function QuickTour:GenerateNextKnockoutRound(winners)
    local tourId = GlobalTournamentSettings.tourId

    local lastRound = ""
    for i=#QuickTourGrouping[tourId],1,-1 do
        if QuickTourGrouping[tourId][i][7] then
            lastRound = QuickTourGrouping[tourId][i][9]
            break
        end
    end

    local nextRound =
        (lastRound=="Round of 16" and "Quarter Final") or
        (lastRound=="Quarter Final" and "Semi Final") or
        (lastRound=="Semi Final" and "Final")

    if not nextRound then return end

    for i=1,#winners,2 do
        table.insert(QuickTourGrouping[tourId], {
            winners[i], winners[i+1], 0,0,false,0,true,false,nextRound,1
        })
    end

    print("✅ "..nextRound.." created following bracket tree")
end
...
function QuickTour:GenerateKnockoutFromGroupStandings()
    ...
    local groups = {}
    for g,_ in pairs(standings) do table.insert(groups, g) end
    table.sort(groups)
    ...
    for _,g in ipairs(groups) do
        local t = {}
        for _,v in pairs(standings[g]) do table.insert(t, v) end
        ...
        winners[#winners+1] = t[1].teamId
        runners[#runners+1] = t[2].teamId
    end
    ...
    for _,m in ipairs(R16) do
        table.insert(QuickTourGrouping[tourId], {
            m[1], m[2], 0,0,false,0,true,false,"Round of 16",1
        })
    end

    print("✅ UEFA style Round of 16 generated")
end
