...
function QuickTour:GenerateNextKnockoutRound(winners)
    ...
    for i = #QuickTourGrouping[tourId], 1, -1 do
        if QuickTourGrouping[tourId][i][7] then
            currentRound = QuickTourGrouping[tourId][i][9]
            break
        end
    end

    local nextRound =
        currentRound == "Round of 16" and "Quarter Final" or
        currentRound == "Quarter Final" and "Semi Final" or
        currentRound == "Semi Final" and "Final" or nil

    if not nextRound then return end

    print("📌 Generating " .. nextRound .. " (Bracket Locked)")
    ...
    for i = 1, #winners, 2 do
        ...
        print("➡️ " .. winners[i] .. " vs " .. winners[i+1])
    end
end
