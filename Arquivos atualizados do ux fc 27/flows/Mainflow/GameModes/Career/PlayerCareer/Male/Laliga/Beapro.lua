...
function Beapro:new(init)
  ...
  o.leagueLogo = { name = "$LeaguePlayer", id = 53 }
...
function Beapro:calculateCareerResult()
  local totalPlayed, win, lose = 0, 0, 0
  ...
  return totalPlayed, win, lose
end

function Beapro:publishFailedLabel()
  if currentBeaproInfo[beaproId].isSuccess == 1 then
    local totalPlayed, win, lose = self:calculateCareerResult()
    local totalAllMatch = #BeaproGrouping[beaproId]

    local text = string.format(
      "You only completed %d out of %d total matches, winning %d and suffering %d defeat",
      totalPlayed,
      totalAllMatch,
      win,
      lose
...
function Beapro:publishFinishLabel()
  if currentBeaproInfo[beaproId].isSuccess == 2 then
    local totalPlayed, win, lose = self:calculateCareerResult()
    local totalAllMatch = #BeaproGrouping[beaproId]
    ...
    local text = string.format(
      "Congratulations, you have completed Player Career mode as a %s player with a perfect winning record. Out of %d matches, you achieved %d wins and %d losses.",
      teamName,
      totalAllMatch,
      win,
      lose
...
function Beapro:publishMatchVisible()
  local visible = true
  if currentBeaproInfo[beaproId].isSuccess == 1 then
    visible = false
  end
...
function Beapro:publishResultVisible()
  local visible = false
  if currentBeaproInfo[beaproId].isSuccess == 1 then
    visible = true
...
function Beapro:publishSuccessVisible()
  local visible = false
  if currentBeaproInfo[beaproId].isSuccess == 2 then
    visible = true
...
function Beapro:PlayMatchday(data)
...
function Beapro:OpponentAnalysis(data)
  activeMatchIndex = data.id
  local m = BeaproGrouping[beaproId][activeMatchIndex]
  currentBeaproData.Index = beaproId
  currentBeaproData.round = activeMatchIndex
