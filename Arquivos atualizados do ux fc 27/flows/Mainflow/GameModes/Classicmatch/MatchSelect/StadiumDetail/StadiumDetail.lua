...
function StadiumDetail:new(init)
  local o = init or {}
  setmetatable(o, self)
  self.__index = self
  o.services = {
...
function StadiumDetail:initOptionsTeam()
...
function StadiumDetail:publishHomeTeamName()
  self.im.Publish(bndHomeTeamName, self.currentTeamName.home)
end
function StadiumDetail:publishAwayTeamName()
  self.im.Publish(bndAwayTeamName, self.currentTeamName.away)
end
function StadiumDetail:initOptionsStadium()
  local currentHomeTeam = currentMatch.HomeTeamID
  if currentHomeTeam == self.awayTeamID then
...
function StadiumDetail:publishStadiumName()
  self.im.Publish(bndStadiumName, self.currentStadium.name)
end
-- Popup Play Match --
function StadiumDetail:PlayMatch()
  local buttonNo = {
    icon = "$FooterIconNo",
    label = "Cancel",
    clickEvents = {
...
function StadiumDetail:finalize()
