...
function P.strip_keys(t, opts)
  opts = opts or {}
  local keyset   = opts.keys or {}
  local patterns = opts.patterns or {}
  local replace  = opts.replace_with 

  local function should_strip(k)
    if keyset[k] then return true end
    local ks = tostring(k):lower()
    for _, p in ipairs(patterns) do
      if ks:match(p) then return true end
    end
    return false
  end
...
--! remove team/nation/playerid fields (versi lama, tetap dipertahankan)
function P.remove_flag_and_crest(t)
    local patterns = {
        "teamid", 
        "nation"   
    ...
    local function strip_recursive(tbl)
        if type(tbl) ~= "table" then return tbl end
        local new = {}
        for k, v in pairs(tbl) do
            local remove = false
            for _, p in ipairs(patterns) do
                if tostring(k):lower():find(p:lower()) then
                    remove = true
                    break
                end
            end
            if not remove then
                new[k] = strip_recursive(v)
            end
        end
        return new
    end

    return strip_recursive(t)
end


--! remove player-related details safely (versi baru, tanpa bug)
function P.remove_detail_player(t)
    ...
    -- rekursif aman (hindari loop referensi)
    local function strip_recursive(tbl, seen)
        if type(tbl) ~= "table" then return tbl end
        ...
        for k, v in pairs(tbl) do
            ...
            for _, p in ipairs(patterns) do
                if keystr:find(p, 1, true) then  -- literal match, bukan regex
                    remove = true
                    break
                end
            end
...
function P.remove_player_attributes(t)
  return P.strip_keys(t, {
    keys = _set({
      "overall","ovr","rating","ratings","attributes","stats",
      "pace","shooting","passing","dribbling","defending","physical",
      "acceleration","sprint_speed","finishing","long_shots","vision",
      "short_passing","ball_control","agility","balance","reactions",
      "interceptions","standing_tackle","sliding_tackle","strength","stamina"
    }),
    patterns = {
      "^attr",  
      "stat",  
