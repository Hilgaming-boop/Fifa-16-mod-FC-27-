...
function TeamRoster:publishPlayerRows()
  -- Acak urutan pemain terlebih dahulu
  shuffleTable(PlayerListData)

  for i, v in ipairs(PlayerListData) do
    local player = v.playerInfo

    v.data.PlayerAvatar = {
      name = "$Head",
      id = v.assetId
    }
    v.data.PlayerName = player.playerName
    v.data.Rating = player.rating
    v.data.clickAction = v.clickAction
    v.data.FontColor = "0xffffff"
    v.data.Position = player.position

    -- Tetap tampilkan bendera negara
