...
function TeamManagementModelAway:comparePlayers(activePlayer, passivePlayer)
  self.playerComparisonData = self:getPlayerComparisonData(activePlayer, passivePlayer)
  self:_publishPlayerComparisonData()
end

function TeamManagementModelAway:swapPlayersByIndex(activeIndex, passiveIndex)
...
function TeamManagementModelAway:getPlayerComparisonData(activePlayer, passivePlayer)
...
function TeamManagementModelAway:_publishStarting11()
...
function TeamManagementModelAway:_publishSubsAndRes()
  if self.players == nil then
    return
  end
...
function TeamManagementModelAway:_publishChemistryMatrix()
...
function TeamManagementModelAway:_publishTeamStats(bindingName)
  ...
  elseif bindingName == bnd_awayTEAM_CREST then
    self.im.Publish(bnd_awayTEAM_CREST, {
      name = "$Crest64x64",
      id = self.teamID
    })
...
function TeamManagementModelAway:_publishPlayerComparisonData()
  self.im.Publish(bnd_awayPLAYER_COMPARISON_DATA, self.playerComparisonData)
end

function TeamManagementModelAway:_publishTeamRatingLabel()
