...
function TeamManagementModel:publishSwapAssets()
  ...
  self.im.Publish("bnd_swap3_active", {
    ...
    self.im.Publish("bnd_swap_label3", "SHOT")
    self.im.Publish("bnd_swap_label4", "PASS")
    self.im.Publish("bnd_swap_label5", "DRI")
    self.im.Publish("bnd_swap_label6", "DEF")
    self.im.Publish("bnd_swap_label7", "PHY")
    self.im.Publish("bnd_swap_notification", "OnlyFans MounTsa")
end
...
function TeamManagementModel:getPlayerComparisonData(activePlayer, passivePlayer)
  ...
  if activePlayer == nil and passivePlayer == nil then
    ...
    for i = 1, 6 do
        comparisonData.attributes[i] = customLabels[i]
...
function TeamManagementModel:finalize()
    ...
    self:clearPlayerVisibility()
    for i = 1, 6 do
        self.im.Unsubscribe(BND_ACTIVE_STAT_PREFIX..i)
        self.im.Unsubscribe(BND_PASSIVE_STAT_PREFIX..i)
    end
    for i = 1, 6 do
        self.im.Unsubscribe(BND_ACTIVE_STAT_COLOR_PREFIX..i)
        self.im.Unsubscribe(BND_PASSIVE_STAT_COLOR_PREFIX..i)
    end
