...
local LeagueIDs = {
  ...
  {ids={15,97,1800,1919,109,1960,1952,1807,1794,1806,12,3,1795,1792,19521,1790,91,1801,88,1947,17,94,95,89},compID=14}, -- EFL Championship 
...
function Liga:publishMatchRows2()
  ...
  local leagueLabelRanges = {
    [13] = { {pos=1,label="Champion"}, {pos=2,label="UEFA Champions League"}, {pos=4,label="UEFA Europa League"}, {pos=6,label="UEFA Conference League"}, {pos=18,label="Relegation"} },
    [53] = { {pos=1,label="Champion"}, {pos=2,label="UEFA Champions League"}, {pos=4,label="UEFA Europa League"}, {pos=6,label="UEFA Conference League"}, {pos=18,label="Relegation"} },
    [31] = { {pos=1,label="Champion"}, {pos=2,label="UEFA Champions League"}, {pos=4,label="UEFA Europa League"}, {pos=6,label="UEFA Conference League"}, {pos=18,label="Relegation"} },
    [19] = { {pos=1,label="Champion"}, {pos=2,label="UEFA Champions League"}, {pos=4,label="UEFA Europa League"}, {pos=6,label="UEFA Conference League"}, {pos=16,label="Relegation"} },
    [16] = { {pos=1,label="Champion"}, {pos=2,label="UEFA Champions League"}, {pos=4,label="UEFA Europa League"}, {pos=6,label="UEFA Conference League"}, {pos=18,label="Relegation"} },
    [11] = { {pos=1,label="Champion"}, {pos=2,label="AFC Champions League Elite"}, {pos=4,label="AFC Champions League Two"}, {pos=6,label="AFC Challenge League"}, {pos=16,label="Relegation"} },    
  }
  ...
  for _, teamID in ipairs(TeamList) do
    ...
    -- Ambil liga ID tim dari LeagueMap
    local compID = LeagueMap[teamID] or 0
    teamData.TeamLigaID = compID

    -- Ambil next opponent
    local nextOpponentID = teamID
    if LigaGroupingList then
      for i = startIndex, endIndex do
        local match = LigaGroupingList[i]
        if match then
          local homeID = match[1]
          local awayID = match[2]
          if teamID == homeID then
            nextOpponentID = awayID
            break
          elseif teamID == awayID then
            nextOpponentID = homeID
            break
          end
        end
      end
  ...
  local defaultRelegation = totalTeams - 2
  if defaultRelegation < 2 then defaultRelegation = totalTeams end

  -- =============================
  -- Sisipkan label (Champion, Relegation, dll)
  -- =============================
  local labeledList = {}
  for i, row in ipairs(teamDataList) do
    ...
    local labelRanges = leagueLabelRanges[row.data.TeamLigaID]
    if labelRanges then
        ...
        if pos == 1 then leagueText = "Champion" end
        if pos == defaultRelegation then leagueText = "Relegation" end
    end
