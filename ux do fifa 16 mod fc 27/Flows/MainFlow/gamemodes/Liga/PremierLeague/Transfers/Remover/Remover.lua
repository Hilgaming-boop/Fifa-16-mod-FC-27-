...
function Remover:publishMatchRows()
    ...
    for _, player in ipairs(teamLineup) do
        -- Only include players who have transfer offers
        if transferOffers[player.CARD_ID] then
            local playerName = player.playerName .. " (Team: " .. (transferOffers[player.CARD_ID].buyingTeamID or 0) .. ", Bid: " .. (transferOffers[player.CARD_ID].transferBid or 0) .. ")"
...
function Remover:Empty()
    ...
    local popupData = {
        title = "NO SQUAD",
        message = "No players available to display",
        buttons = {buttonYes}
    }
    self.nav.Event(nil, "evt_show_popup", popupData)
end
