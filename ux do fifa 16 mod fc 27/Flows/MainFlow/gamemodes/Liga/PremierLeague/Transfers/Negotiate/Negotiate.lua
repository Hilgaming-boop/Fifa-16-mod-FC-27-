...
League.Config = {
    ...
    BidAdjustments = {
        MinBid = { Id = 1, Step = 500000, Binding = "bnd_min_bid" },
        MaxBid = { Id = 2, Step = 10000000, Binding = "bnd_max_bid" },
        MinBuy = { Id = 3, Step = 1000000, Binding = "bnd_min_buy" },
        MaxBuy = { Id = 4, Step = 5000000, Binding = "bnd_max_buy" }
    },
    Actions = {
    ...
    Bindings = {
        ...
        PlayerName = "bnd_player_name",
        PlayerAge = "bnd_player_age",
        OfferPrice = "bnd_offer_price",
        PlayerPosition = "bnd_player_position",
        PlayerRating = "bnd_player_rating",
        OurTeamCrest = "bnd_our_team_crest"
    }
}
...
local function getPlayerInfo(playerId, teamId)
    local squadService = function() 
        return { GetCurrentPlayerLineup = function(_, team, _) 
            return {
                { CARD_ID = 1001, playerName = "Lionel Messi", rating = 93, position = "RW", jerseyNumber = 10 },
                { CARD_ID = 1002, playerName = "Cristiano Ronaldo", rating = 91, position = "ST", jerseyNumber = 7 }
            }
        end }
    end
    
    local players = squadService().GetCurrentPlayerLineup(0, teamId, 0) or {}
...
function League:new(options)
    ...
    instance.api = instance.api or function(service) 
        ...
        return { 
            GetCurrentOptions = function() return {} end, 
            GetCurrentPlayerLineup = function(_, team, _) 
                return {
                    { CARD_ID = 1001, playerName = "Lionel Messi", rating = 93, position = "RW", jerseyNumber = 10 },
                    { CARD_ID = 1002, playerName = "Cristiano Ronaldo", rating = 91, position = "ST", jerseyNumber = 7 }
                }
    ...
    instance.loc = instance.loc or { 
    LocalizeInteger = function(num) 
        return formatNumberWithCommas(num) 
    end, 
    LocalizeString = function(str) return str end 
}
    ...
    instance.teamVisuals = {
        ...
        ourTeamCrest = { name = "$Crest", id = currentSelectedTeamID or 2 }
    }
    ...
    local playerInfo = getPlayerInfo(GLOBAL_PLAYERINSTALL or 1001, GLOBAL_TEAMBUY or 1)
    instance.playerStats = { 
        rating = playerInfo and playerInfo.rating or 50, 
        position = playerInfo and playerInfo.position or "CM", 
        age = 25, 
        name = playerInfo and playerInfo.playerName or "Unknown Player",
...
function League:fetchPlayerStats()
    ...
    for _, player in ipairs(players) do
        if player.CARD_ID == (GLOBAL_PLAYERINSTALL or 1001) then
            ...
            self.playerStats.name = player.playerName or "Unknown Player"
            self.playerStats.playerInfo = player
            print("Player stats updated:", self.playerStats)
            break
        end
    end
end
...
function League:subscribeToEvents()
    ...
    local eventBindings = {
        ...
        League.Config.Bindings.PlayerName, League.Config.Bindings.PlayerAge,
        League.Config.Bindings.OfferPrice, League.Config.Bindings.PlayerPosition,
        League.Config.Bindings.PlayerRating, League.Config.Bindings.OurTeamCrest
    }
    for _, binding in ipairs(eventBindings) do
        self.im.Subscribe(binding, function() 
            self:publishMatchLabel() 
            self:publishPlayerDetails()
        end)
    end
...
function League:setupBindings()
    ...
    self.im.Publish(binding, { value = 1, locValue = formatNumberWithCommas(value) })
    ...
    self.im.Subscribe(League.Config.Bindings.OfferPrice, function() 
    local offerValue = WAGEMAX or self.bidValues.maxBuy or 0
    self.im.Publish(League.Config.Bindings.OfferPrice, "$" .. formatNumberWithCommas(offerValue))
    print("Published offer price:", offerValue)
end)
    
    self.im.Subscribe(League.Config.Bindings.PlayerName, function() 
        self:fetchPlayerStats()
        local playerName = self.playerStats.name or "Unknown Player"
        self.im.Publish(League.Config.Bindings.PlayerName, playerName)
        print("Published player name:", playerName)
    end)
end

function League:publishPlayerDetails()
    print("Publishing player details...")
    
    self:fetchPlayerStats()
    
    local playerName = self.playerStats.name or "Unknown Player"
    self.im.Publish(League.Config.Bindings.PlayerName, playerName)
    print("Player name published:", playerName)
    
    self.im.Publish(League.Config.Bindings.PlayerAge, tostring(self.playerStats.age or 25))
    
    self.im.Publish(League.Config.Bindings.PlayerPosition, self.playerStats.position or "CM")
    
    self.im.Publish(League.Config.Bindings.PlayerRating, tostring(self.playerStats.rating or 50))
    
    local offerValue = WAGEMAX or self.bidValues.maxBuy or 0
    self.im.Publish(League.Config.Bindings.OfferPrice, "$" .. formatNumberWithCommas(offerValue))
    
    self.im.Publish(League.Config.Bindings.OurTeamCrest, self.teamVisuals.ourTeamCrest)
end
...
function League:setupBidActions()
    ...
    self.im.Publish(League.Config.Bindings.OfferPrice, "$" .. formatNumberWithCommas(WAGEMAX))
end
...
function League:updateBidPlaceholders()
    ...
    self.im.Publish(League.Config.Bindings.OfferPrice, "$" .. formatNumberWithCommas(WAGEMAX or 0))
end
...
function League:publishMatchLabel()
    ...
    self.im.Publish(League.Config.Bindings.TransferValue, "$" .. formatNumberWithCommas(TVALUE))
self.im.Publish(League.Config.Bindings.TeamMatchProgress, "$" .. formatNumberWithCommas(GLOBAL_FUNDS))
