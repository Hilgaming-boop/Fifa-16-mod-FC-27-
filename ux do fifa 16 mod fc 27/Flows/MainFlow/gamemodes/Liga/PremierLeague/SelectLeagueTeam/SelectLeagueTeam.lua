...
local BND_CURRENT_COUNTRY_NAME = "bnd_current_country_name"
...
function SelectLeagueTeam:new(init)
    ...
    o.im.Subscribe(BND_CURRENT_COUNTRY_NAME, function()
        if o.currentLeagueID then
            local countryInfo = o.services.CountryService.GetCountryInfoByFUTLeagueId(o.currentLeagueID)
            o.im.Publish(BND_CURRENT_COUNTRY_NAME, countryInfo and countryInfo.name or "")
        end
    end)
...
function SelectLeagueTeam:setSelectedLeagueIndexHelper(index)
    if self.leagueIndex ~= index then
        ...
        self.im.Refresh(BND_CURRENT_COUNTRY_NAME)
