...
function Brain:generateAutomaticFixtures(teams)
    ...
    -- Jika tim ganjil, tambahkan BYE agar round robin tetap jalan
    if numTeams % 2 ~= 0 then
        table.insert(teams, "BYE")
        numTeams = numTeams + 1
    end
    
    local function canPlayAtGround(team, isHome)
        if team == "BYE" then return true end
    ...
    for i = 1, numTeams - 1 do
        ...
        for j = 1, numTeams / 2 do
            ...
            -- Abaikan jika ada BYE (tim libur)
        ...
        -- rotasi tim kecuali index pertama
