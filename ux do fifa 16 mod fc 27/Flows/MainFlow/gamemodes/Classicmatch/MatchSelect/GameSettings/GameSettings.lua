...
function GameSettings:initOptionsTeam()
...
function GameSettings:publishHomeTeamName()
  self.im.Publish(bndHomeTeamName, self.currentTeamName.home)
end
function GameSettings:publishAwayTeamName()
  self.im.Publish(bndAwayTeamName, self.currentTeamName.away)
end
function GameSettings:initOptionsStadium()
  local currentHomeTeam = currentMatch.HomeTeamID
  if currentHomeTeam == self.awayTeamID then
...
function GameSettings:publishStadiumName()
  self.im.Publish(bndStadiumName, self.currentStadium.name)
end
-- Popup Play Match --
function GameSettings:PlayMatch()
  local buttonNo = {
    icon = "$FooterIconNo",
    label = "Cancel",
    clickEvents = {
...
function GameSettings:finalize()
