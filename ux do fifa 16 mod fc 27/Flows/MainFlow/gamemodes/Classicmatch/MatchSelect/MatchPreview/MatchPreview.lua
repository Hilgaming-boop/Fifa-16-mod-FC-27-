...
function MatchPreview:new(init)
  local o = init or {}
  setmetatable(o, self)
  self.__index = self
  o.services = {
...
function MatchPreview:initOptionsTeam()
...
function MatchPreview:publishHomeTeamName()
  self.im.Publish(bndHomeTeamName, self.currentTeamName.home)
end
function MatchPreview:publishAwayTeamName()
  self.im.Publish(bndAwayTeamName, self.currentTeamName.away)
end
function MatchPreview:initOptionsStadium()
  local currentHomeTeam = currentMatch.HomeTeamID
  if currentHomeTeam == self.awayTeamID then
...
function MatchPreview:publishStadiumName()
  self.im.Publish(bndStadiumName, self.currentStadium.name)
end
-- Popup Play Match --
function MatchPreview:PlayMatch()
  local buttonNo = {
    icon = "$FooterIconNo",
    label = "Cancel",
    clickEvents = {
...
function MatchPreview:finalize()
