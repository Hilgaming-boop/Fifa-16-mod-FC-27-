...
function ControllerSettings:new(init)
  local o = init or {}
  setmetatable(o, self)
  self.__index = self
  o.services = {
...
function ControllerSettings:initOptionsTeam()
...
function ControllerSettings:publishHomeTeamName()
  self.im.Publish(bndHomeTeamName, self.currentTeamName.home)
end
function ControllerSettings:publishAwayTeamName()
  self.im.Publish(bndAwayTeamName, self.currentTeamName.away)
end
function ControllerSettings:initOptionsStadium()
  local currentHomeTeam = currentMatch.HomeTeamID
  if currentHomeTeam == self.awayTeamID then
...
function ControllerSettings:publishStadiumName()
  self.im.Publish(bndStadiumName, self.currentStadium.name)
end
-- Popup Play Match --
function ControllerSettings:PlayMatch()
  local buttonNo = {
    icon = "$FooterIconNo",
    label = "Cancel",
    clickEvents = {
...
function ControllerSettings:finalize()
