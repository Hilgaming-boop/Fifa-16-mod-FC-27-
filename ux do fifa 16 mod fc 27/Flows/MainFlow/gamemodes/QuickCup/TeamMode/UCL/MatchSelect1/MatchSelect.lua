...
for i = 1, 30 do
  ...
  o.im.Subscribe("bnd_cupmodelbg", function()
    o.im.Publish("bnd_cupmodelbg", o.cupmodelbg)
  end)
  ...
  o.im.Subscribe("bnd_my_team", function()
    o:publishMatchInfo()
  end)
...
function MatchSelect:publishMatchInfo()
  if self.cupData.championCrest.id == 0 and self.cupData.isFinish == false then
    ...
    self.im.Publish("bnd_home_team", self.loc.LocalizeString("TeamName_Abbr3_"..self.cupData.homeTeamCrest.id))
    self.im.Publish("bnd_away_crest", self.cupData.awayTeamCrest)
    self.im.Publish("bnd_away_team", self.loc.LocalizeString("TeamName_Abbr3_"..self.cupData.awayTeamCrest.id))
    self.im.Publish("bnd_my_team", self.loc.LocalizeString("TeamName_Abbr15_"..self.cupData.homeTeamCrest.id))
  end
end

function MatchSelect:GetMatchAwayTeamId()
  local currentCupGrouping = QuickCupGrouping[cupId]
  local index = 0
...
function MatchSelect:GetMatchRound(index)
...
function MatchSelect:StartMatch()
  local buttonNo = {
    icon = "$FooterIconNo",
    label = "Cancel",
    clickEvents = {
...
function MatchSelect:_openHelpPopup()
  local titleText = ""
  local messageText = "How does the tournament work?                                         Tournaments work as follows; Choose 1/16 available teams. you will start the tournament in the round of 16, your objective is to win 4/4 matches to be the champion. Good luck!"
  local buttonClose = {
    icon = "$FooterIconNo",
    label = "Close",
    clickEvents = {
...
function MatchSelect:finalize()
  ...
  self.im.Unsubscribe("bnd_modelcup")
  ...
  self.im.Unsubscribe("bnd_my_team")
  for k,v in pairs(bndList) do
