...
function QuickTour:GenerateKnockoutFromGroupStandings()
    ...
    -- 2. Kumpulkan Tim dari Semua Grup (A-F)
    local allWinners = {}      
    local allRunnersUp = {}    
    local allThirdPlaces = {}  
    local groups = {"A", "B", "C", "D", "E", "F"}

    for _, letter in ipairs(groups) do
        local g = standings["GROUP " .. letter]
        local t = {}
        for _, v in pairs(g) do 
            v.originGroup = letter 
            table.insert(t, v) 
        end
        table.sort(t, sortTeamsLogic)

        table.insert(allWinners, t[1])
        table.insert(allRunnersUp, t[2])
        if t[3] then table.insert(allThirdPlaces, t[3]) end
    end

    -- 3. Urutkan Ranking (Juara 1-6 & Peringkat 3 Terbaik 1-4)
    table.sort(allWinners, sortTeamsLogic)
    table.sort(allThirdPlaces, sortTeamsLogic)

    local best3rd = {}
    for i = 1, 4 do table.insert(best3rd, allThirdPlaces[i]) end

    local runnerUpPool = {}
    for _, r in ipairs(allRunnersUp) do table.insert(runnerUpPool, r) end

    -- 4. HELPER ANTI-REMATCH (Cek Grup Asal)
    local function getValidOpponent(pool, excludeGroup)
        -- Cari tim yang grupnya TIDAK sama dengan excludeGroup
        for i, team in ipairs(pool) do
            if team.originGroup ~= excludeGroup then
                return table.remove(pool, i)
            end
        end
        -- Jika terpaksa (hanya terjadi jika algoritma macet), ambil yang pertama
        return table.remove(pool, 1)
    end

    local r16_pairings = {}

    -- TAHAP A: 4 Juara Terbaik vs 4 Peringkat 3 Terbaik
    for i = 1, 4 do
        local winner = allWinners[i]
        local opponent = getValidOpponent(best3rd, winner.originGroup)
        table.insert(r16_pairings, {winner, opponent})
    end

    -- TAHAP B: 2 Juara Sisanya vs 2 Runner-up
    for i = 5, 6 do
        local winner = allWinners[i]
        local opponent = getValidOpponent(runnerUpPool, winner.originGroup)
        table.insert(r16_pairings, {winner, opponent})
    end

    -- TAHAP C: 4 Runner-up Sisanya vs Satu Sama Lain (Tanpa Reuni Grup)
    -- Kita urutkan agar runner-up dengan grup berbeda dipasangkan
    while #runnerUpPool >= 2 do
        local team1 = table.remove(runnerUpPool, 1)
        local opponent = getValidOpponent(runnerUpPool, team1.originGroup)
        table.insert(r16_pairings, {team1, opponent})
    end

    -- 5. Daftarkan Match ke Sistem
    for _, pair in ipairs(r16_pairings) do
        table.insert(QuickTourGrouping[tourId], {  
    ...
    print("- Juara 1-4 vs P3 (Beda Grup)")
    print("- Juara 5-6 vs RU (Beda Grup)")
    print("- RU vs RU (Beda Grup)")
end
