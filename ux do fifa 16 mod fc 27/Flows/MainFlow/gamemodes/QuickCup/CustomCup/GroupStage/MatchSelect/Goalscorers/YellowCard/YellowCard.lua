...
function Tour:publishMatchRows()
    print("⚙️ Publishing ONLY Yellow Card rows...")
    ...
    -------------------------------------------------------
    -- LOAD HANYA PLAYER DENGAN KARTU KUNING
    -------------------------------------------------------
    ...
    for teamID, players in pairs(TeamPlayerCache[tourId]) do
        for _, player in ipairs(players) do
            if player and player.CARD_ID then
                ...
                local yellowCount = TournamentStats[tourId].YellowCards[playerID] or 0

                -- FILTER: HANYA pemain dengan kartu kuning > 0
    ...
    -------------------------------------------------------
    -- SORTING (Paling banyak kartu kuning di atas)
    -------------------------------------------------------
    table.sort(allPlayerStats, function(a, b)
        if a.yellowCards ~= b.yellowCards then return a.yellowCards > b.yellowCards end
        return a.playerName < b.playerName
    end)

    -------------------------------------------------------
    -- KIRIM KE UI (HANYA YELLOW CARD)
    -------------------------------------------------------
    local uiDataList = {}
    ...
    print(" Published " .. #uiDataList .. " players to the Yellow Card screen.")
end
