...
function Tour:new(init)
    ...
    o:publishMatchRows()
    o:UpdateModeLabelText() -- Memicu teks L/K/G di awal layar dibuka

    return o
end

-- Fungsi tambahan untuk teks Mode Dinamis
function Tour:UpdateModeLabelText()
    local labelText = "T" -- Default untuk Group + Knockout
    
    if GlobalTournamentSettings then
        if GlobalTournamentSettings.isLeagueMode then
            labelText = "L" -- Untuk League
        elseif GlobalTournamentSettings.isKnockoutOnly then
            labelText = "T" -- Untuk Knockout Only
        end
    end
...
function Tour:setupSubscriptions()
    ...
    --  Publish warna 2 (Typo diperbaiki dari tourIdToColorMap2 ke tourIdToColorTabMap)
...
function Tour:publishMatchRows()
    print("⚙️ Publishing ONLY Goalscorers rows...")
    ...
    for teamID, players in pairs(TeamPlayerCache[tourId]) do
        for _, player in ipairs(players) do
            if player and player.CARD_ID then
                ...
                local goalsCount = TournamentStats[tourId].Goals[playerID] or 0

                -- FILTER: HANYA pemain dengan gol > 0
    ...
    -------------------------------------------------------
    -- KIRIM KE UI (DATA KARTU DIHAPUS TOTAL)
    -------------------------------------------------------
    local uiDataList = {}
    for rank, p in ipairs(allPlayerStats) do
        table.insert(uiDataList, {
            data = {
                ...
                Goals = p.goals,
                -- YellowCard dan RedCard dihapus agar tidak terkirim ke UI
                Rank = tostring(rank),
    ...
    print(" Published " .. #uiDataList .. " goalscorers to the screen.")
end
