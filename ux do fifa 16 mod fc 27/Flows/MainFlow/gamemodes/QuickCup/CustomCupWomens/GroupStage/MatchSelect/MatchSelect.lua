...
function QuickTour:GenerateKnockoutFromGroupStandings()
    ...
    -- cari last matchday grup
    local lastGroupMatchday = 0
    for _, m in ipairs(QuickTourGrouping[tourId]) do
        if m[8] and m[10] > lastGroupMatchday then
            lastGroupMatchday = m[10]
        end
    end

    local SEMI_MATCHDAY = lastGroupMatchday + 3
    local FINAL_MATCHDAY = SEMI_MATCHDAY + 1

    local function sortGroup(g)
        local t = {}
        for _, v in pairs(g) do table.insert(t, v) end
    ...
    local A = sortGroup(standings["GROUP A"])
    local B = sortGroup(standings["GROUP B"])

    -- SEMI FINAL (langsung)
    table.insert(QuickTourGrouping[tourId], {
        A[1].teamId, B[2].teamId,
        0, 0, false, 0,
        true, false, "Semi Final", SEMI_MATCHDAY
    })

    table.insert(QuickTourGrouping[tourId], {
        B[1].teamId, A[2].teamId,
        0, 0, false, 0,
        true, false, "Semi Final", SEMI_MATCHDAY
    })

    print("✅ Direct Semi Final @ matchday "..SEMI_MATCHDAY)
end
