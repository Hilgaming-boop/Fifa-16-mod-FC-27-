...
function Dev:new(init)
    ...
    Dev.options = {
        devType = {
            ...
            { name = " MOUNTSA ", value = 2 },
            { name = " LAOSIJI ", value = 3 },
            { name = " MA'RUF ID ", value = 4 },
            { name = " VOLTA SPORT ", value = 5 },
            { name = " MVNPROD ", value = 6 },
            { name = " MELDA14 ", value = 7 },
            { name = " NB PLAYS ", value = 8 },
            { name = " AF'ID ", value = 9 },
            { name = " ZHINXI ", value = 10 },
            { name = " CROT DI MULUT ", value = 11 },
            { name = " HNFH16 ", value = 12 },
            { name = " ROBER FL ", value = 13 },
            { name = " TALAMLEGIT ", value = 14 },
            { name = " PRAS 87 ", value = 15 },
            { name = " AKHMAD FARIS ", value = 16 },
            { name = " RIANPROSS ", value = 17 },
            { name = " DAVUNPES ", value = 18 },
            { name = " INISIAL A ", value = 19 }            
        }
    }
...
function Dev:ValidateInput()
    if self.manualInputText and self.manualInputText ~= "" then
        if string.len(self.manualInputText) <= 15 and string.len(self.manualInputText) >= 2 then
    ...
    if self.showManualInput then
        if self.isInputValid then
            ...
            if self.manualInputText == "" then
                ...
                self.im.Publish(BND_NAME_VALID, "Name 2-15 Characters")
            end
        end
    else
        ...
        if typeOpt.value == 1 then
            if self.isInputValid then
                ...
                if self.manualInputText == "" then
                    ...
                    desc = "Name must be 2–15 characters"
                end
            end
        else
