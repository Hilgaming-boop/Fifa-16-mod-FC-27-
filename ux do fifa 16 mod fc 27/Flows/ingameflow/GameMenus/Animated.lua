...
function Animated:_publishCount()
  -- matikan semua panel 1–10
  for i = 1, 10 do
    self.im.Publish("bnd_panel" .. i .. "_visible", false)
  end
  -- matikan semua animasi 0–10
  for i = 0, 10 do
    self.im.Publish("bnd_animate_count" .. i, false)
  end

  local panelId = 12 - self.count  -- supaya panel1 muncul saat count=11
  if panelId >= 1 and panelId <= 10 then
