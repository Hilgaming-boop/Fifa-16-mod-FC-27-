...
WorldCupInfo = {
  ...
  bnd_background_show = true,
  bnd_background_height = 120,
  bnd_background_width = 924,
  bnd_background = {
    name = "$Event",
    id = "CupID2"
  },
  bnd_background_bottom = -5,
  bnd_color_alpha = 0,
  bnd_home_rect_left = -50,
  bnd_home_rect_top = 20,
  bnd_away_rect_left = -50,
  ...
  bnd_message_fontSize = 16,
  bnd_message_fontColor = "0xffffff",
  bnd_awaymessage_right = 390,
  bnd_message_text_width = 250,
  bnd_awaymessage_top = -55,
  bnd_awaymessage_left = 395,
...
function GameEvents:updateClock(msg)
  ...
  if self.isKickoffDone == nil then
      self.isKickoffDone = false
      self.kickoffTimer = os.time()
      
    
      local hId = self.TeamsData[1] and self.TeamsData[1].assetId or 0
      local aId = self.TeamsData[2] and self.TeamsData[2].assetId or 0
      local hName = self.TeamsData[1] and (self.TeamsData[1].teamName or self.TeamsData[1].name or self.TeamsData[1].abbrevName) or "HOME"
      local aName = self.TeamsData[2] and (self.TeamsData[2].teamName or self.TeamsData[2].name or self.TeamsData[2].abbrevName) or "AWAY"
      
      local msgKickoff = hId .. "|" .. hName .. "|" .. aId .. "|" .. aName .. "|0|0|||||"
      self:updateGameEvents(0, "SHOW", "KICKOFF", msgKickoff)
      
      self.currentdata.bnd_homecrest.id = hId
      self.currentdata.bnd_awaycrest.id = aId
      
      self.currentdata.bnd_homemessage_visible = false
      self.currentdata.bnd_awaymessage_visible = false
      self.currentdata.bnd_homemessage_text = ""
      self.currentdata.bnd_awaymessage_text = ""
      
      self.currentdata.bnd_title_text = "KICK-OFF"
      
      self:_publishEventInfo()
      --if self.currentEvent.bnd_logo then self.im.Publish("bnd_logo", self.currentEvent.bnd_logo) end
      --if self.currentEvent.bnd_background then self.im.Publish("bnd_background", self.currentEvent.bnd_background) end
  ...
  if self.isKickoffDone == false and self.kickoffTimer then
      if os.time() - self.kickoffTimer >= 6 then
          self.isKickoffDone = true
          self.kickoffTimer = nil
  ...
  self.currentGameTime = currentGameTime  
    ...
    if self.eventShowStartTime then
        local showElapsed = os.time() - self.eventShowStartTime
        local displayDuration = 7
        if showElapsed >= displayDuration then
            self.eventShowStartTime = nil
            self.isHiding = true
            self.animTarget = 0
            self.animating = true
            print("Auto hiding event based on real duration (extratime fix)")
        end
    end
...
function GameEvents:updateGameEvents(subtype, hideshow, subtypestr, msg)
    ...
    if hideshow == "SHOW" then
        ...
        if params and table.getn(params) > 0 then
            ...
            self.im.Publish(BND_SCALE_Y, 0.75)

            self.eventShowStartTime = os.time()
            ...
            self.currentdata.bnd_homename_short = self.loc.LocalizeString("TeamName_Abbr3_" .. tostring(gameevents.homeTeamId))
            self.currentdata.bnd_awayname_short = self.loc.LocalizeString("TeamName_Abbr3_" .. tostring(gameevents.awayTeamId))
...
function GameEvents:getPlayerInfo(teamSide, playername, isOg)
    ...
    if playername and string.find(playername, "%-") then
    ...
    for i = 1, #teamlineupData do
        local player = teamlineupData[i]

        if player and player.playerName then
            local lineupName = player.playerName

            if string.find(lineupName, "%-") then
                lineupName = string.gsub(lineupName, "%-", " ")
            end

            if string.find(playername, lineupName, 1, true)
                or string.find(lineupName, playername, 1, true) then

                found = true

                playerInfo.assetId = player.CARD_ID or 0
                playerInfo.playerAvatar = player.CARD_ID or 0
                playerInfo.playerName = player.playerName
                playerInfo.level = player.level or 0
                playerInfo.nationalityID = player.nationalityID or 0
    ...
    if found and isOg then
        found = false
    elseif (not found) and isOg then
        found = true
    end

    if not found then
        playerInfo.assetId = 0
        playerInfo.playerAvatar = 0
    end
